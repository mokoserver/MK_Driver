...
# --- Защиты (для BK Precision синтаксис может отличаться) --------------------
MOKO.Driver(PSU, "set", "ocp = ON")                 # Защита по току (OCP)
MOKO.Driver(PSU, "set", "SCPI = CURR:PROT:STAT ON") # Прямая SCPI (BK синтаксис)

MOKO.Driver(PSU, "set", "ovp = ON")                 # Защита по напряжению (OVP)
MOKO.Driver(PSU, "set", "SCPI = VOLT:PROT:STAT ON") # Прямая SCPI (BK синтаксис)
